...
class UKF:
    ...
    def resetUKF(self, _Q, _R, x_0):
        ...
        self.mngm = MNGM2(self.n, x_0) # this is the model of the system, which has to be defined in a separate class, here we use the one from the EKF, but it can be any other model as well, as long as it has the same interface (state and output functions)
