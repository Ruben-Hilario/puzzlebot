...
AutonomousObstacleAvoidance::AutonomousObstacleAvoidance() : Node("autonomous_obstacle_avoidance_node") {
    // Create subscription to laser scan
    auto qos = rclcpp::SensorDataQoS();
    scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
        "/scan",qos,std::bind(&AutonomousObstacleAvoidance::scanCallback, this, std::placeholders::_1)
    );
    ...
    state_pub = this->create_publisher<std_msgs::msg::String>("state_pub", 10);

    path_sub_ = this->create_subscription<nav_msgs::msg::Path>(
        "/planned_path", 10, std::bind(&AutonomousObstacleAvoidance::pathCallback, this, std::placeholders::_1)
    );
    ...
    RCLCPP_INFO(this->get_logger(), "Autonomous Obstacle Avoidance Node initialized with FSM");
}

void AutonomousObstacleAvoidance::scanCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    const auto& ranges = msg->ranges;
    const float angle_increment = msg->angle_increment;
    const float angle_min = msg->angle_min;
    const int num_ranges = ranges.size();
...
float AutonomousObstacleAvoidance::getMinDistanceInRange(const sensor_msgs::msg::LaserScan& scan,
                                               float angle_min, float angle_max) {
    const auto& ranges = scan.ranges;
    const float scan_angle_min = scan.angle_min;
    const float angle_increment = scan.angle_increment;
...
void AutonomousObstacleAvoidance::publishVelocity(float linear_x, float angular_z) {
    auto twist = geometry_msgs::msg::Twist();
    twist.linear.x = linear_x;
    twist.linear.y = 0.0;
    twist.linear.z = 0.0;
...
void AutonomousObstacleAvoidance::publishDistanceInfo(float front_distance, float left_distance, float right_distance, float back_distance) {
    std_msgs::msg::String msg;
    msg.data = "Front: " + std::to_string(front_distance) + " m, Left: " + std::to_string(left_distance) + " m, Right: " + std::to_string(right_distance) + " m, Back: " + std::to_string(back_distance) + " m";
    dis_pub->publish(msg);
}

void AutonomousObstacleAvoidance::publishDebugInfo(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    // Extract and publish scans for each sector
    auto back_scan = extractScanInRange(*msg, -FRONT_ANGLE, FRONT_ANGLE);
    auto left_scan = extractScanInRange(*msg, LEFT_ANGLE_START, LEFT_ANGLE_END);
    auto right_scan = extractScanInRange(*msg, RIGHT_ANGLE_START, RIGHT_ANGLE_END);
...
sensor_msgs::msg::LaserScan AutonomousObstacleAvoidance::extractScanInRange(const sensor_msgs::msg::LaserScan& scan,
                                                                   float angle_min, float angle_max) {
    sensor_msgs::msg::LaserScan extracted_scan;
...
void AutonomousObstacleAvoidance::pathCallback(const nav_msgs::msg::Path::SharedPtr msg) {
    if (!msg->poses.empty()) {
        auto_state = AutoState::PATH;
        RCLCPP_INFO(this->get_logger(), "Received new path, following path.");
    }
}

void AutonomousObstacleAvoidance::stopRobot() {
    publishVelocity(0.0, 0.0);
}

void AutonomousObstacleAvoidance::updateFSMState(float front_distance, float left_distance, float right_distance, double time_delta) {
    std_msgs::msg::String state_msg;

    switch (auto_state) {
        case AutoState::IDLE:
            accumulated_distance += LINEAR_SPEED * time_delta;
            if (accumulated_distance < 0.10f) { // move ~10 cm
                publishVelocity(LINEAR_SPEED, 0.0);
                state_msg.data = "idle";
            } else {
                stopRobot();
                auto_state = AutoState::STOPPED;
        ...
        case AutoState::PATH:
            if (front_distance < OBSTACLE_DISTANCE_THRESHOLD) {
                ...
                auto_state = AutoState::AVOIDING;
                accumulated_angle = 0.0f;
                state_msg.data = "avoiding";
                RCLCPP_INFO(this->get_logger(), "Obstacle detected. Evading.");
                if (left_distance > right_distance) {
                ...
                state_msg.data = "path";
                // While following path, do not publish velocity here to avoid overriding path follower
            }
            break;

        case AutoState::AVOIDING:
            state_msg.data = "avoiding";
            accumulated_angle += ANGULAR_SPEED * time_delta;
            
            if (front_distance > OBSTACLE_DISTANCE_THRESHOLD + 0.1f && accumulated_angle > TURN_ANGLE_TARGET) {
                // Evaded successfully, stop and wait for a new path
                auto_state = AutoState::STOPPED;
                stopRobot();
                state_msg.data = "stopped";
                RCLCPP_INFO(this->get_logger(), "Evaded obstacle. Stopped and waiting for new path.");
            } else {
                // Continue evading
                if (left_distance > right_distance) {
                    publishVelocity(0.0, ANGULAR_SPEED);
