...
void MonteCarlo::scanCb(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    sensorModel(msg); 
     ...
     // 2. Resample particles to keep the good ones and drop the bad ones
    //resampleParticles();
...
void MonteCarlo::resampleParticles() {
    std::vector<Particle> new_particles;
    int index = std::uniform_int_distribution<int>(0, num_particles_ - 1)(gen_);
    double beta = 0.0;
    
    auto max_it = std::max_element(particles_.begin(), particles_.end(),
        [](const Particle& a, const Particle& b) { return a.weight < b.weight; });
    double max_weight = max_it->weight;
    ...
    // Reset weights uniformly for the next iteration
...
//     // 1. Update particle weights based on the laser scan
//     // NOTE: You must update sensorModel to read from your own custom 'grid_' 
//     // instead of 'map_.data' since 'map_' is currently commented out!
//     sensorModel(msg); 

//     // 2. Resample particles to keep the good ones and drop the bad ones
//     resampleParticles();

//     // 3. Find the Best Particle (now it will actually mean something!)
//     auto best_it = std::max_element(particles_.begin(), particles_.end(), 
//         [](const Particle& a, const Particle& b) { return a.weight < b.weight; });
...
//             // Inside your sensorModel loop, replace the map check with:
//             if (mx >= 0 && mx < map_width_ && my >= 0 && my < map_height_) {
//                 int map_val = grid_[my * map_width_ + mx];
//                 if (map_val == 100) {
//                     p_weight *= 2.0;  // High match!
//                 } else if (map_val == -1) {
//                     p_weight *= 1.0;  // Neutral / Unknown
//                 } else {
//                     p_weight *= 0.2;  // Penalty: Expected wall, found free space
//                 }
//             } else {
//                 p_weight *= 0.01;     // Penalty: Out of bounds
//             }
//         }
